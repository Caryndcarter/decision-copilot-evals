/**
 * Smoke test for lib/db/runs.ts and lib/db/users.ts against MongoDB Atlas
 * (or whatever MONGODB_URI / DB_NAME point at). No LLM calls; no Next.js server needed.
 *
 * Run from repo root:
 *   env-cmd -- npx tsx packages/nextjs/scripts/smoke-runs-db.ts
 *
 * Cleans up its own data on success.
 */
...
async function runRunsTests(): Promise<{ insertedRunIds: string[] }> {
  ...
  check(
    "getRun stamps createdAt",
    typeof (fetched1 as unknown as { createdAt?: string } | null)?.createdAt === "string"
  );
  check(
    "getRun stamps updatedAt on insert",
    typeof (fetched1 as unknown as { updatedAt?: string } | null)?.updatedAt === "string"
  );
...
async function main() {
  console.log("Smoke test: MongoDB persistence layer");
  console.log("DB_NAME:", process.env.DB_NAME ?? "decision-copilot-evals");
  ...
  try {
    ...
    if (failed === 0) {
      ...
      for (const id of cleanupUserIds) {
        if (await deleteUserById(id)) userCleaned += 1;
      }
