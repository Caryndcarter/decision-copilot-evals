...
export function SignUpBlocked({ message }: { message: string }) {
  return (
    <AuthShell>
      <div className="rounded-xl border border-zinc-200 bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold text-zinc-900 tracking-tight">Invitation required</h1>
...
export function SignUpForm({
  ...
  const expiresLabel = new Date(expiresAtIso).toLocaleString(undefined, {
    dateStyle: "medium",
    timeStyle: "short",
  });
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      const res = await fetch("/api/auth/invite/prepare", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = (await res.json()) as { ok?: boolean; error?: string };
      if (!res.ok || !data.ok) {
        setError(data.error || "Invitation could not be verified.");
        setGoogleLoading(false);
        return;
      }
      await signIn("google", { callbackUrl: "/runs" });
    } catch {
      setError("Something went wrong. Please try again.");
      setGoogleLoading(false);
    }
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1.5 text-sm text-zinc-500">
            Invitation valid until {expiresLabel}
          </p>
        </div>
