/**
 * ARCHIVED demo dataset — infrastructure migrate decision (Vercel → AWS).
 *
 * This is the original product-tour scenario. It is no longer the active tour
 * (see `tour-demo-data.ts` for the selected dataset). Kept in-repo so it can be
 * swapped back in by changing the single re-export line in the barrel. Do not
 * delete without an intentional decision to retire the Vercel/AWS story.
 *
...
export const TOUR_SCENARIO: TourScenario = {
  decisionId: "demo-vercel-aws-migration",
  label: "Vercel vs self-hosted AWS",
};
...
export const TOUR_UNIFIED_BRIEF: TourUnifiedBrief = {
  ...
  contributions: [
    { provider: "anthropic", note: "Full TCO and Vercel optimization before irreversible spend" },
    { provider: "openai", note: "Traffic-split pilot and rollback runbook" },
    { provider: "gemini", note: "Hybrid routing matrix and residency narrative" },
  ],
};
